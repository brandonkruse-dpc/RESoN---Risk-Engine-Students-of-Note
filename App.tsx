...
import { Student, RiskWeights, RiskLevel, BackendConfig } from './types';
import { DEFAULT_WEIGHTS, Icons } from './constants';
import { MOCK_STUDENTS } from './services/mockData';
import { calculateRiskScore, getRiskLevel } from './services/riskEngine';
import { syncFromManageBac, fetchLatestFromDrive, BRIDGE_CODE_TEMPLATE } from './services/managebacSync';
import { RiskBadge } from './components/RiskBadge';
import { WeightSlider } from './components/WeightSlider';
import { generateStudentRiskSummary, generateBatchReport } from './services/geminiService';
import { 
      ...
      if (bridgeUrl && backendConfig.gDriveRawFolder) {
        try {
          ...
          if (latest && latest.length > 0) {
            ...
            // In production, we'd map MB raw student data to our Student type
            // setStudents(latest); 
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <aside className="w-64 bg-indigo-950 text-indigo-100 flex flex-col shadow-xl z-20">
        <div className="p-6">
          ...
          <p className="text-[10px] text-indigo-400 mt-1 uppercase tracking-widest font-bold italic text-center">Student Risk Engine</p>
        </div>
        ...
        <nav className="flex-1 px-4 space-y-2">
          ...
          <button onClick={() => setActiveTab('students')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'students' ? 'bg-indigo-800 text-white shadow-lg' : 'hover:bg-indigo-900/50'}`}>
            <Icons.Students />
            <span className="font-medium text-sm text-left">Students Tracking</span>
          </button>
          <button onClick={() => setActiveTab('reports')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'reports' ? 'bg-indigo-800 text-white shadow-lg' : 'hover:bg-indigo-900/50'}`}>
            <Icons.Report />
            <span className="font-medium text-sm text-left">PDF Reports</span>
          </button>
          ...
          <button onClick={() => setActiveTab('backend')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'backend' ? 'bg-amber-600 text-white shadow-lg' : 'hover:bg-indigo-900/50'}`}>
            <Icons.Cloud />
            <span className="font-medium text-sm text-left">Sync Engine</span>
          </button>
          <button onClick={() => setActiveTab('settings')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'settings' ? 'bg-indigo-800 text-white shadow-lg' : 'hover:bg-indigo-900/50'}`}>
            <Icons.Settings />
            <span className="font-medium text-sm text-left">Risk Weights</span>
          </button>
        </nav>
        ...
        <div className="p-4 mt-auto border-t border-indigo-900/50">
          <div className="flex items-center gap-3 p-3 bg-indigo-900/40 rounded-xl">
            ...
            <div className="overflow-hidden">
              <p className="text-xs font-bold text-white truncate">DP Coordinator</p>
      ...
      <main className="flex-1 overflow-y-auto bg-slate-50 relative">
        <header className="sticky top-0 bg-white/80 backdrop-blur-md border-b border-slate-200 px-8 py-4 z-10 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <input 
                type="text" 
                placeholder="Query student by name/ID..." 
                value={searchTerm}
        ...
        <div className="p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="md:col-span-1 bg-indigo-900 text-white p-8 rounded-[2rem] shadow-xl relative overflow-hidden">
                   <div className="relative z-10">
                    <p className="text-[10px] font-black text-indigo-300 uppercase tracking-widest mb-2">Live Cloud Backend</p>
                    <p className="text-4xl font-black">{students.length}</p>
                    <p className="text-[10px] text-indigo-300 mt-2 font-bold uppercase tracking-tighter">Connected Records</p>
                   </div>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 border-l-4 border-l-indigo-500">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">DP1 (Y11) Avg Risk</p>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 border-l-4 border-l-purple-500">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">DP2 (Y12) Avg Risk</p>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 border-l-4 border-l-rose-500">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Critical Alerts</p>
                  <p className="text-3xl font-black text-rose-600">{students.filter(s => s.riskLevel === RiskLevel.HIGH).length}</p>
                </div>
              </div>
              
              {/* Rest of UI... (Omitting identical parts for brevity in response) */}
            </div>
          )}

          {activeTab === 'backend' && (
            <div className="max-w-5xl mx-auto space-y-12">
              <div className="bg-indigo-50 border border-indigo-200 p-8 rounded-[2rem] flex gap-6 items-start shadow-sm">
                 ...
                 <div className="flex-1">
                    ...
                    <p className="text-xs text-indigo-800 leading-relaxed mb-6">
                      RE:SoN uses a <strong>Bridge Proxy</strong> to bypass CORS and securely talk to ManageBac. 
                      You must deploy the script below to Google Apps Script as a Web App.
                    </p>
                    <div className="flex gap-4">
                       ...
                       <a href="https://script.google.com" target="_blank" className="bg-white border-2 border-indigo-200 text-indigo-700 px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-50 transition-all">
                         Open Apps Script
                       </a>
                    </div>
                 </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-slate-100">
                  <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-8">1. Connection Settings</h3>
                  <div className="space-y-6">
                    <div>
                      ...
                      <input 
                        ...
                        className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 py-3 text-sm focus:border-indigo-500 outline-none transition-all"
                      />
                    </div>
                    <div>
                      ...
                      <input 
                        type="text" 
                        value={backendConfig.mbDomain} 
                        onChange={e => setBackendConfig({...backendConfig, mbDomain: e.target.value})}
                        className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 py-3 text-sm focus:border-indigo-500 outline-none transition-all"
                      />
                    </div>
                    <button 
                      onClick={handleManualSync}
                      disabled={isSyncing || !bridgeUrl}
                      className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 text-white font-black py-4 rounded-2xl text-xs uppercase tracking-widest shadow-xl shadow-indigo-100 transition-all"
                    >
                ...
                <div className="bg-slate-900 text-white p-10 rounded-[2.5rem] shadow-2xl">
                  <h3 className="text-sm font-black text-indigo-300 uppercase tracking-widest mb-8 text-center">2. Security & Storage</h3>
                  <div className="space-y-6">
                    <div>
                      ...
                      <input 
                        type="password" 
                        value={backendConfig.mbApiKey} 
                        onChange={e => setBackendConfig({...backendConfig, mbApiKey: e.target.value})}
                        className="w-full bg-slate-800 border-none rounded-2xl px-5 py-3 text-sm mono text-indigo-100 focus:ring-2 focus:ring-indigo-500 outline-none text-center"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-2 tracking-widest text-center">Drive Folder ID/URL</label>
                      <input 
                        type="text" 
                        value={backendConfig.gDriveRawFolder} 
                        onChange={e => setBackendConfig({...backendConfig, gDriveRawFolder: e.target.value})}
                        className="w-full bg-slate-800 border-none rounded-2xl px-5 py-3 text-xs text-indigo-100 focus:ring-2 focus:ring-indigo-500 outline-none"
                      />
                    </div>
                    <div className="p-4 bg-white/5 rounded-xl border border-white/10 text-center">
                       <p className="text-[9px] text-indigo-300 italic">
                         Authorization is granted when you deploy the script. Only users with access to this specific Bridge URL and ManageBac Key can retrieve data.
                       </p>
                    </div>
                  </div>
                </div>
          ...
          {/* Settings and Reports tabs... */}
        </div>
      </main>

      {/* Bridge Modal and other modals... (identical to previous version) */}
      {showBridgeCode && (
               ...
               <div className="relative group">
                <div className="absolute top-4 right-4 z-10">
                   <button 
                    ...
                    className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${copied ? 'bg-emerald-500 text-white' : 'bg-slate-800 text-white hover:bg-black'}`}
                   >
